import MAIL_ACTIONS from '../enums/MailActions.js';
...
const sendVerifyOtp = async (req, res) => {
    try {
        ...
        const userData = {
            verifyOtp: otp,
            verifyOtpExpiredAt: new Date(Date.now() + 2.5 * 60 * 1000).toISOString(), // expires at 2 minutes 30 seconds from now
        };

        const updatedUser = await adminService.updateById(user.admin_id, userData);
        if (!updatedUser) {
        ...
        const mailAction = MAIL_ACTIONS.VERIFY;
...
const verifyEmail = async (req, res) => {
    try {
        ...
        await adminService.updateById(user.admin_id, updateUserData);
...
const forgotPassword = async (req, res) => {
    try {
        ...
        const userData = {
            resetOtp: otp,
            resetOtpExpiredAt: new Date(Date.now() + 2.5 * 60 * 1000).toISOString(), // expires at 2 minutes 30 seconds from now
        };
        ...
        const mailAction = MAIL_ACTIONS.RESET;
